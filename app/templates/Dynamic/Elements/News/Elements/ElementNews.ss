<div class="news-layout layout-$Layout  clearfix <% if $bg_image %> has-bg-image <% else %> no-bg-image <% end_if %> text-$text_width  pad-top-$pad_top pad-bot-$pad_bot vheight-$vheight text-on-$BackgroundColor bg-pattern-$BackgroundPattern bg-gradient-$BackgroundGradient  <% if $BoxedText %>boxedtext<% end_if %> <% if $Style %>$StyleVariant<% end_if %>"> 
    ...
    <div class="inner inner-$inner_wrap grid  $layout_class clearfix">
        <div class="col">
            <div id="news-{$ID}">
				...
				<div class="layer text-on-left text-35  overflow">
					<div class="bg-image background-image">
					</div>
					...
					<div class="inner inner-full-width grid clearfix">
						<div class="col overflow">
							<div id="newsCarousel1" class="news-carousel-wrapper ">
								<ul id="fakeNews" class="list-unstyled services carousel news-carousel swiper-wrapper">
									<script>
										var content = '';
										...
										var elem = document.querySelector('#fakeNews');
										// Set HTML content
